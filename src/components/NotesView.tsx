...
interface NotesViewProps { onBack: () => void; }
...
export default function NotesView({ onBack }: NotesViewProps) {
    ...
    return (
      <div className="px-5 pb-24 animate-fadeIn">
        <div className="flex items-center gap-3 mb-6 pt-5">
          <button onClick={() => setEditing(null)} className="w-10 h-10 rounded-xl flex items-center justify-center cursor-pointer" style={{ background: "#151A25", border: "1px solid #1E2532" }}><ArrowLeft size={20} color="#fff" /></button>
        ...
        <div className="rounded-2xl p-4 min-h-[200px]" style={{ background: "#151A25", border: "1px solid #1E2532" }}>
          <input value={editing.title} onChange={(e) => setEditing({ ...editing, title: e.target.value })} placeholder="Note title..." className="w-full bg-transparent border-none outline-none text-lg font-semibold mb-3" style={{ color: "#fff" }} />
          <textarea value={editing.body} onChange={(e) => setEditing({ ...editing, body: e.target.value })} placeholder="Write your thoughts..." className="w-full bg-transparent border-none outline-none resize-none text-[15px] leading-relaxed min-h-[140px]" style={{ color: "#D1D5DB" }} />
        </div>
  ...
  return (
    <div className="px-5 pb-24 animate-fadeIn">
      <div className="flex items-center gap-3 mb-6 pt-5">
        <button onClick={onBack} className="w-10 h-10 rounded-xl flex items-center justify-center cursor-pointer" style={{ background: "#151A25", border: "1px solid #1E2532" }}><ArrowLeft size={20} color="#fff" /></button>
