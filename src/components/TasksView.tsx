...
interface TasksViewProps { onBack: () => void; }
  ...
  return (
    <div className="px-5 pb-24 animate-fadeIn">
      ...
      <div className="flex gap-2.5 mb-5">
        ...
        <button onClick={addTask} className="w-12 h-12 rounded-[14px] flex items-center justify-center cursor-pointer" style={{ background: "#1E3A5F", color: "#6BB5FF" }}><Plus size={24} /></button>
      </div>
      ...
      {tasks.map((task) => (
        <div key={task.id} className="flex items-center gap-3.5 p-4 rounded-2xl mb-2.5 transition-all" style={{ background: "#151A25", border: "1px solid #1E2532", opacity: task.done ? 0.4 : 1 }}>
          <button onClick={() => toggleTask(task.id)} className="w-[22px] h-[22px] rounded-lg flex items-center justify-center flex-shrink-0 cursor-pointer" style={{ border: task.done ? "2px solid #6BB5FF" : "2px solid #3A4558", background: task.done ? "#6BB5FF" : "transparent" }}>{task.done && <Check size={14} color="#0B0E14" strokeWidth={3} />}</button>
